...
{   
    public class SoftradeService : ISoftradeService
    {
        ...
        public ReporteadorResponse ExecuteReportes(ReporteadorRequest request)
        {
            ...
            {                
                //request = GenerarRequestReporte();
                response = controller.ExcuteReportes(request);
            }
            return response;
        }

        private ReporteadorRequest GenerarRequestReporte()
        {
            return new ReporteadorRequest()
            {
                IdUsuarioEjecucion = 14,
                Item = new ReporteadorEntidad()
                {
                    IdEmpresa = 10,
                    FiltroPedimento = new FiltroPedimento()
                    {
                        IdEmpresa = 10,
                        //Pedimento = "5020481"                       
                    }, 
                    NombreReporte = Reporte.OperacionNivelPartida                    
                }
            };
        }
