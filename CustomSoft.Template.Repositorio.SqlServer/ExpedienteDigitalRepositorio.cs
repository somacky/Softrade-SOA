...
namespace CustomSoft.Template.Repositorio.SqlServer
{
    public class ExpedienteDigitalRepositorio : IExpedienteDigitalRepositorio
    {
        ...
        /// <summary>
        /// Funcion que inserta en tabla expediente digital, 
        /// puede insertar pedimentos Archivos M, facturas XML, Archivos PDF e incluso fotos de artículos.
        /// </summary>
        /// <param name="expediente">Entidad de tipo Expediente Digital a insertar</param>
        /// <returns></returns>
        ...
        public ListaExpedienteDigital ObtenerListaDeArchivosTop5Pedimento(ListaExpedienteDigital request, int idUsuario)
        {
            var listaExpedienteDigital = new List<ExpedienteDigital>();
            var parametros = new List<SqlParameterItem>();            
            parametros.Add(new SqlParameterItem("@pPagina", SqlDbType.SmallInt, request.ListaPaginacion.Pagina));
            ...
            helper = InicializaConexion(request.IdEmpresa);
            var reader = helper.ExecuteReader("[usp_ExpedienteDigital_DameListaXTop5XPedimento]", parametros);
            while (reader.Read())
            {
                listaExpedienteDigital.Add(new ExpedienteDigital()
                {
        ...
        public ListaExpedienteDigital ObtenerListaDeArchivosTop5CuentaGasto(ListaExpedienteDigital request, int idUsuario)
        {
            var listaExpedienteDigital = new List<ExpedienteDigital>();
            var parametros = new List<SqlParameterItem>();
            parametros.Add(new SqlParameterItem("@pPagina", SqlDbType.SmallInt, request.ListaPaginacion.Pagina));
            ...
            //parametros.Add(new SqlParameterItem("@pIdTipoDocumento", SqlDbType.SmallInt, idTipoDocumento));
            helper = InicializaConexion(request.IdEmpresa);
            var reader = helper.ExecuteReader("[usp_ExpedienteDigital_DameListaXTop5XCuentaGasto]", parametros);
            while (reader.Read())
            {
                listaExpedienteDigital.Add(new ExpedienteDigital()
                {
