...
namespace CustomSoft.Template.Servicios.Seguridad.Contratos
{
    ...
    public interface ISoftradeService
    {
        ...
        [OperationContract]
        ArticuloDocumentoResponse SubirDocumentoArticulo(ArticuloDocumentoRequest request);

        [OperationContract]
        OperacionListaPedimentoResponse OperacionListadoPedimentos(OperacionPedimentoRequest request);

        [OperationContract]
        TokenResponse OperacionToken(TokenRequest request);

        [OperationContract]
        UrlTemporalResponse OperacionUrlTemporal(UrlTemporalRequest request);

        [OperationContract]
        CuentaGastoGetListResponse GetListCuentaGasto(CuentaGastoGetListRequest request);

        [OperationContract]
        TasaCambioResponse GetTasaCambio(TasaCambioRequest request);

        [OperationContract]
        EstadoClimaResponse GetEstadoClima(EstadoClimaRequest request);

        [OperationContract]
        OperacionesGraficasResponse GetDashboard(OperacionesGraficasRequest request);
    }
}
